...
private[rdd] object ParallelFileMerger extends Logging {
    ...
    @tailrec def addToBlock(
      ...
      if (bytesInFile < 0) {
        ...
        if (currentBlock.isEmpty)
          ...
          (currentBlockIdx, currentBlock.reverse) :: blocks
      } else {
  ...
  def getFullSize(fs: FileSystem,
    ...
    val fileSizes = files.map(path => fs.getFileStatus(path).getLen())
  ...
  def mergeFiles(sc: SparkContext,
    ...
    val (_, sizeByFile) = getFullSize(fs, allFiles)
    ...
    rdd.foreach {
      merge ⇒
        ...
        val (gzipAtEnd, cramAtEnd) =
          if (idx == numBlocksToWrite - 1)
