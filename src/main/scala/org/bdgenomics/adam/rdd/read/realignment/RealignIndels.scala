...
private[read] object RealignIndels extends Serializable with Logging {
  ...
  def mapTargets(
    ...
    val readsMappedToTarget =
      rich_rdd
        .groupBy(
          (r: RichAlignmentRecord) ⇒
            mapToTarget(
              r,
...
private[read] class RealignIndels(
  ...
  def extractSequenceAndQuality(
    ...
    val richCigar = RichCigar(read.samtoolsCigar)
  ...
  def realignTargetGroup(targetGroup: (Option[(Int, IndelRealignmentTarget)], Iterable[RichAlignmentRecord]),
    ...
    if (target.isEmpty) {
      ...
      try {
        ...
        val finalReads = if (readsToClean.nonEmpty && consensus.nonEmpty) {
          ...
          if (lodImprovement > lodThreshold) {
            FinalizingRealignments.time {
              ...
              val cleanedReads: Iterable[RichAlignmentRecord] = readsToClean.map {
                case (r, _) ⇒
                  ...
                  try {
                    ...
                    if (finalRemapping != -1) {
                      ...
                      val (basesClippedAtStart, basesClippedAtEnd) = if (unclipReads) {
                        ...
                        val richCigar = RichCigar(r.samtoolsCigar)
                        (richCigar.softClippedBasesAtStart, richCigar.softClippedBasesAtEnd)
                      }

                      // get start, end, cigar
                      val (newStart, newEnd, newCigar) =
                        cigarAndCoordinates(
                      ...
                      if (newEnd <= newStart) {
                        log.warn(s"Realigning read $r failed because realignment issued an illegal alignment: start $newStart, end $newEnd, CIGAR $newCigar")
                        r
                      } else {
  ...
  def cigarAndCoordinates(readLength: Int,
    ...
    val (consensusStartIdx, consensusEndIdx) = if (consensus.consensus.isEmpty) {
      ...
      (rawStartIdx, rawStartIdx + consensus.consensus.length + 1)
    }
    ...
    if ((remappingIdx + alignedBases) <= consensusStartIdx) {
      ...
      val (basesBefore, basesConsumed, eventOperator, endOffset) =
        if (consensus.consensus.isEmpty)
          (
