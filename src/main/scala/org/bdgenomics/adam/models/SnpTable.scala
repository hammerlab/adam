...
class SnpTable private[models] (
    private[models] val indices: Map[ContigName, (Int, Int)],
    private[models] val sites: Array[Long])
  extends Serializable
    with Logging {

  private val midpoints: Map[ContigName, Int] = {
...
object SnpTable {
   ...
   */
  def apply(variants: VariantRDD)(implicit factory: Factory): SnpTable = CreatingKnownSnpsTable.time {
    val (indices, positions) = CollectingSnps.time {
      ...
      val contigIndices =
        sortedVariants
          .map(v ⇒ v.getContigName: ContigName)
          .zipWithIndex
