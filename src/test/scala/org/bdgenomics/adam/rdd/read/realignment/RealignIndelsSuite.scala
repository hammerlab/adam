...
import org.bdgenomics.adam.models.{ RecordGroupDictionary, ReferencePosition, ReferenceRegion, SequenceDictionary, SequenceRecord }
import org.bdgenomics.adam.rdd.ADAMContext._
import org.bdgenomics.adam.rdd.read.AlignmentRecordRDD
import org.bdgenomics.adam.rdd.read.realignment.RealignIndels.{ getReferenceFromReads, mapTargets, mapToTarget }
...
class RealignIndelsSuite
  ...
  sparkTest("checking mapping to targets for artificial reads") {
    ...
    assert(
      readsMappedToTarget.forall {
        case (target: Option[(Int, IndelRealignmentTarget)], reads: Seq[AlignmentRecord]) ⇒
          reads.forall {
            read ⇒
              if (read.getStart <= 25) {
  ...
  sparkTest("checking extraction of reference from reads") {
    ...
    val targets = RealignmentTargetFinder(artificialReads.map(RichAlignmentRecord(_))).toArray
    val rr = artificialReads.map(RichAlignmentRecord(_))
    val readsMappedToTarget: Array[((Int, IndelRealignmentTarget), Iterable[AlignmentRecord])] = mapTargets(rr, targets)
      ...
      case ((_, target), reads) ⇒
        if (!target.isEmpty) {
          val referenceFromReads: (String, Long, Long) =
            getReferenceFromReads(
              reads
                .map(new RichAlignmentRecord(_))
                .toSeq
            )
  ...
  test("we shouldn't try to realign a region with no target") {
    ...
    assert(
      ri
        .realignTargetGroup(
          (
            None.asInstanceOf[Option[(Int, IndelRealignmentTarget)]],
            reads
          )
        )
        .size === 2
  ...
  sparkTest("realign a read with an insertion that goes off the end of the read") {
    ...
    val rdd = AlignmentRecordRDD(sc.parallelize(Seq(insRead,
      ...
      scRead,
      ecRead)),
      new SequenceDictionary(Vector(SequenceRecord("1", 20))),
