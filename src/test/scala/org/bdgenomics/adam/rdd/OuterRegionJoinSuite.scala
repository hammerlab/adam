...
trait OuterRegionJoinSuite
  ...
  sparkTest("Ensure same reference regions get passed together") {
    ...
    val rdd1 = sc.parallelize(Seq(record1, record3)).keyBy(ReferenceRegion.unstranded)
    val rdd2 = sc.parallelize(Seq(record2, record4)).keyBy(ReferenceRegion.unstranded)
