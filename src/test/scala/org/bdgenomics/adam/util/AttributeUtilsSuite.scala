...
class AttributeUtilsSuite extends FunSuite {
  ...
  test("parseTags returns a reasonable set of tagStrings") {
    ...
    ==(tags.head.tag, "XT")
    ==(tags.head.tagType, TagType.Integer)
    ==(tags.head.value, 3)

    ==(tags(1).tag, "XU")
    ==(tags(1).tagType, TagType.String)
    ==(tags(1).value, "foo,bar")
  }
  ...
  test("oq string tag with many ':' in it is correctly parsed") {
    ...
    ==(tags.size, 1)
    ==(tags.head.value, "C55/15D:::::::.7GFFAFDA442.40F=AGHHE")
  }
  ...
  test("oq string tag with a ',' in it is correctly parsed") {
    ...
    ==(tags.size, 1)
    ==(tags.head.value, "C,55/15D:::::::.7GFFAFDA442.40F=AGHHE")
  }
...
class AttributeSuite extends FunSuite {
  ...
  test("test SAMTagAndValue parsing") {
    ...
    // Simple tag types
    ==(convertSAMTagAndValue(createSAMTagAndValue("XY:i:3")), Attribute("XY", TagType.Integer, 3))
    ==(convertSAMTagAndValue(createSAMTagAndValue("XY:Z:foo")), Attribute("XY", TagType.String, "foo"))
    ==(convertSAMTagAndValue(createSAMTagAndValue("XY:f:3.0")), Attribute("XY", TagType.Float, 3.0f))
    ==(convertSAMTagAndValue(createSAMTagAndValue("XY:A:a")), Attribute("XY", TagType.Character, 'a'))
  ...
  test("Attributes can be correctly re-encoded as text SAM tags") {
    ==(Attribute("XY", TagType.Integer, 3).toString, "XY:i:3")
    ==(Attribute("XY", TagType.String, "foo").toString, "XY:Z:foo")
    ==(Attribute("XY", TagType.Float, 3.0f).toString, "XY:f:3.0")
    ==(Attribute("XY", TagType.Character, 'a').toString, "XY:A:a")

    val intArray = Array(1, 2, 3)
    ==(Attribute("XY", TagType.NumericIntSequence, intArray).toString, "XY:B:i,1,2,3")

    val shortArray: Array[Short] = Array(1, 2, 3).map(_.toShort)
    ==(Attribute("XY", TagType.NumericShortSequence, shortArray).toString, "XY:B:s,1,2,3")

    val floatArray = Array(1.0f, 2.0f, 3.0f)
    ==(Attribute("XY", TagType.NumericFloatSequence, floatArray).toString, "XY:B:f,1.0,2.0,3.0")

    val byteArray: Array[Byte] = Array(1, 2, 3).map(_.toByte)
    ==(Attribute("XY", TagType.NumericByteSequence, byteArray).toString, "XY:B:c,1,2,3")

    // Unsigned int arrays, note the capitalized leading character in the value
    ==(Attribute("XY", TagType.NumericUnsignedByteSequence, byteArray).toString, "XY:B:C,1,2,3")
    ==(Attribute("XY", TagType.NumericUnsignedIntSequence, intArray).toString, "XY:B:I,1,2,3")
    ==(Attribute("XY", TagType.NumericUnsignedShortSequence, shortArray).toString, "XY:B:S,1,2,3")
