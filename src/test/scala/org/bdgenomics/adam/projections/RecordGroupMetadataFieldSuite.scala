...
class RecordGroupMetadataFieldSuite extends ADAMFunSuite {
  ...
  test("Use projection when reading parquet record group metadata") {
    ...
    ==(recordGroupMetadata.count(), 1)
    ==(recordGroupMetadata.first.getName, "name")
    ==(recordGroupMetadata.first.getSequencingCenter, "sequencing_center")
    ==(recordGroupMetadata.first.getDescription, "description")
    ==(recordGroupMetadata.first.getRunDateEpoch, 42L)
    ==(recordGroupMetadata.first.getFlowOrder, "flow_order")
    ==(recordGroupMetadata.first.getKeySequence, "key_sequence")
    ==(recordGroupMetadata.first.getLibrary, "library")
    ==(recordGroupMetadata.first.getPredictedMedianInsertSize, 99)
    ==(recordGroupMetadata.first.getPlatform, "platform")
    ==(recordGroupMetadata.first.getPlatformUnit, "platform_unit")
  }
}
