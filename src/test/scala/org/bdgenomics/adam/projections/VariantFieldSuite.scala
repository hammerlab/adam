...
class VariantFieldSuite extends ADAMFunSuite {
  ...
  test("Use projection when reading parquet variants") {
    ...
    ==(variants.count(), 1)
    ==(variants.first.getContigName, "6")
    ==(variants.first.getStart, 29941260L)
    ==(variants.first.getEnd, 29941261L)
    ==(variants.first.getNames.get(0), "rs3948572")
    ==(variants.first.getReferenceAllele, "A")
    ==(variants.first.getAlternateAllele, "T")
    ==(variants.first.getFiltersApplied, true)
    ==(variants.first.getFiltersPassed, false)
    ==(variants.first.getFiltersFailed.get(0), "filter")
  }
}
